...
| T5 | Elevation | Member account creates a role that bypasses guardrails | Med | High | SCP denies IAM changes to `awlz-*` roles; permission boundary required for role creation; `awlz-role-permissions-boundary` Config rule detects any customer role without it | Applied 2026-07-30 in four member accounts. The rule sees roles, not the SCP: a role created before the boundary existed is reported, not blocked |
...
| T8 | DoS | Guardrails lock out legitimate emergency access | Low | Med | Documented break-glass role + procedure; CloudWatch alarm on assumption of the four exact recovery-role ARNs, notifying an SNS topic under a dedicated CMK | Alarm proved live on 2026-07-30 by real assumptions, not a synthetic metric. The alarm reports use; it cannot distinguish legitimate recovery from abuse |
...
| T5 | Config rule on role creation without boundary — live since 2026-07-30 |
| T8 | CloudWatch alarm on break-glass role assumption — live and observed firing 2026-07-30 |
